    ...
    <div className="mt-3 pt-3 border-t border-gray-200/60 dark:border-slate-600/60">
      ...
      {isEditing || isAddingNote ? (
        <textarea
          ...
          className="w-full text-xs text-gray-700 dark:text-slate-200 bg-white/50 dark:bg-slate-700/50 border border-gray-200 dark:border-slate-600 rounded p-2 focus:outline-none focus:ring-1 focus:ring-indigo-500 resize-none"
          rows={2}
          autoFocus={isAddingNote}
        />
      ) : (
        <div className="text-xs text-gray-600 dark:text-slate-300 bg-white/50 dark:bg-slate-700/50 rounded p-2 border border-transparent whitespace-pre-wrap">
          {notes}
        </div>
      )}
    </div>
